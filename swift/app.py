...
def history_detail(qr_id):
    ...
    visitor_sessions = c.execute(
        'SELECT visitor_name, reason, created_at FROM visitor_sessions WHERE qr_id = ? ORDER BY created_at DESC',
        (qr_id,)
    ).fetchall()
    conn.close()
...
def visitor_chat_start(vehicle_token):
    ...
    get_visitor_session(row['id'], session[visitor_session_key], visitor_name)
...
def upload_image():
    ...
    try:
        ...
        c.execute(
            ...
            (row['id'], sender_type, sender_name, '', image_path, timestamp)
        )
        conn.commit()
    ...
    socketio.emit('receive_message', payload, room=f'chat_{vehicle_token}')
